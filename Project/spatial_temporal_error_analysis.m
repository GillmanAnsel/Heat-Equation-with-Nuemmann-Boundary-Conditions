...
% Plot 1: L2 norm evolution
subplot(2,2,1);
plot(T_fine, L2_norm, 'b-', 'LineWidth', 2.5);
hold on;
plot(T_fine, sqrt(1/4) * exp(-T_fine), 'r--', 'LineWidth', 2, 'DisplayName', '$\frac{1}{2}e^{-t}$ (exact $\|u\|_{L^2}$)');
xlabel('Time t', 'FontSize', 11);
ylabel('$\|u(\cdot,t)\|_{L^2}$', 'FontSize', 11, 'Interpreter', 'latex');
title('L² Norm Evolution for $u(x,t) = e^{-t}\sin(\pi x)$', 'FontSize', 12, 'Interpreter', 'latex');
grid on;
...
subplot(2,2,2);
plot(T_fine, max_norm, 'g-', 'LineWidth', 2.5);
hold on;
plot(T_fine, exp(-T_fine), 'r--', 'LineWidth', 2, 'DisplayName', '$e^{-t}$ (exact $\|u\|_\infty$)');
xlabel('Time t', 'FontSize', 11);
ylabel('$\max_x |u(x,t)|$', 'FontSize', 11, 'Interpreter','latex');
title('Max Norm Evolution for $u(x,t) = e^{-t}\sin(\pi x)$', 'FontSize', 12, 'Interpreter', 'latex');
grid on;
legend('FEM Solution', 'Theoretical decay', 'FontSize', 10, 'Interpreter', 'latex');
hold off;
...
title('Solution $u(x,t) = e^{-t}\sin(\pi x)$ Heatmap', 'FontSize', 12, 'Interpreter', 'latex');
colormap(gca, 'hot');
...
sgtitle('Solution Dynamics and Error Evolution for $u(x,t) = e^{-t}\sin(\pi x)$', 'FontSize', 14, 'Interpreter', 'latex');
