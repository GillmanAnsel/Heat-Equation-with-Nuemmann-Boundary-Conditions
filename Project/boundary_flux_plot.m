...
g_left_func  = @(t) pi;       % Left Neumann BC: du/dx at x=0
g_right_func = @(t) -pi;      % Right Neumann BC: du/dx at x=L
...
plot(T, flux_left, 'b-', 'LineWidth', 2.5, 'DisplayName', 'Left BC: $g_{left}(t) = \pi$ at $x=0$');
hold on;
plot(T, flux_right, 'r-', 'LineWidth', 2.5, 'DisplayName', 'Right BC: $g_{right}(t) = -\pi$ at $x=L$');
...
title('Boundary Heat Flux Over Time for $u(x,t) = e^{-t}\sin(\pi x)$', 'FontSize', 14, 'Interpreter', 'latex');
grid on;
...
title('Boundary Heat Flux Magnitude for $u(x,t) = e^{-t}\sin(\pi x)$ Over Time', 'FontSize', 14, 'Interpreter', 'latex');
grid on;
legend('FontSize', 11, 'Location', 'best', 'Interpreter', 'latex');
xlim([0, Tf]);
