...
du_exact = @(x,t) pi * exp(-t) * cos(pi*x);  % Gradient = π e^{-t} cos(πx)
...
for frame_idx = 1:length(time_frames)
    ...
    title(sprintf('Heat Flux Field for $u(x,t) = e^{-t}\\sin(\\pi x)$: $\\frac{\\partial u}{\\partial x} = \\pi e^{-t}\\cos(\\pi x)$ at t = %.4f (Nx = %d)', T(n), Nx), ...
